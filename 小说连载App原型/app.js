...
const APP_MODIFIED_AT = '2026年08月24日 01:28';
...
function openItemPreview(type, index) { const item = scoped(type === 'material' ? 'materials' : 'outlines')[index]; if (!item) return; const preview = document.querySelector('#itemPreview'); document.querySelector('#itemPreviewMeta').textContent = type === 'material' ? `${item.type || '素材'} · ${activeBook}` : `剧情大纲 · ${activeBook}`; document.querySelector('#itemPreviewTitle').textContent = item.title || '未命名'; document.querySelector('#itemPreviewInfo').textContent = type === 'material' ? (item.tags ? `# ${item.tags}` : '无标签') : `${item.status || '待写'} · ${item.chapter || '未关联章节'}`; document.querySelector('#itemPreviewContent').textContent = type === 'material' ? (item.content || '暂无内容') : (item.note || '暂无补充说明'); preview.hidden = false; }
function closeItemPreview() { document.querySelector('#itemPreview').hidden = true; }
...
function renderOutlines() {
  const form = document.querySelector('#outlineForm'); form.hidden = true; editingOutlineIndex = null;
  const items = scoped('outlines'); document.querySelector('#outlineWorkContext').textContent = contextName();
  document.querySelector('#outlineList').innerHTML = activeBook && items.length ? items.map((item, index) => `<article class="outline-item previewable" data-outline-preview="${index}" tabindex="0" role="button"><div class="outline-item-head"><h3>${escapeHtml(item.title)}</h3><span>${escapeHtml(item.status)}</span></div><p>${escapeHtml(item.note || '暂无补充说明')}</p><small>${escapeHtml(item.chapter || '未关联章节')}</small><div class="item-actions"><button class="outline-edit" data-outline-index="${index}">编辑</button><button class="outline-delete" data-outline-index="${index}">删除</button></div></article>`).join('') : `<div class="material-empty"><b>◇</b>${activeBook ? '还没有情节，点击右上角新增一条吧。' : '请先选择一部作品。'}</div>`;
  document.querySelectorAll('[data-outline-preview]').forEach(item => { item.addEventListener('click', event => { if (!event.target.closest('.item-actions')) openItemPreview('outline', Number(item.dataset.outlinePreview)); }); item.addEventListener('keydown', event => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); openItemPreview('outline', Number(item.dataset.outlinePreview)); } }); });
  document.querySelectorAll('.outline-edit').forEach(button => button.addEventListener('click', event => { event.stopPropagation(); openOutlineForm(Number(button.dataset.outlineIndex)); }));
  document.querySelectorAll('.outline-delete').forEach(button => button.addEventListener('click', event => { event.stopPropagation(); requestDeletion('outline', Number(button.dataset.outlineIndex)); }));
  document.querySelector('#outlineCount').textContent = activeBook ? (items.length ? `${items.length} 条情节` : '还没有情节') : '选择作品后查看';
}
...
function renderMaterials() {
  const items = scoped('materials'); const list = (activeMaterialFilter === '全部' ? items : items.filter(item => item.type === activeMaterialFilter)).map(item => ({ item, index: items.indexOf(item) }));
  document.querySelector('#materialList').innerHTML = activeBook && list.length ? list.map(({ item, index }) => `<article class="material-item previewable" data-material-preview="${index}" tabindex="0" role="button"><span class="material-item-head"><span class="material-type">${escapeHtml(item.type)}</span><span class="material-work">${escapeHtml(activeBook)}</span></span><h3>${escapeHtml(item.title)}</h3><p>${escapeHtml(item.content)}</p>${item.tags ? `<span class="material-tags"># ${escapeHtml(item.tags)}</span>` : ''}<div class="item-actions"><button class="material-edit" data-material-index="${index}">编辑</button><button class="material-delete" data-material-index="${index}">删除</button></div></article>`).join('') : `<div class="material-empty"><b>◇</b>${activeBook ? `还没有${activeMaterialFilter === '全部' ? '素材' : activeMaterialFilter + '素材'}，点击右上角新增一条吧。` : '请选择或创建一部作品。'}</div>`;
  document.querySelectorAll('[data-material-preview]').forEach(item => { item.addEventListener('click', event => { if (!event.target.closest('.item-actions')) openItemPreview('material', Number(item.dataset.materialPreview)); }); item.addEventListener('keydown', event => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); openItemPreview('material', Number(item.dataset.materialPreview)); } }); });
  document.querySelectorAll('.material-edit').forEach(button => button.addEventListener('click', event => { event.stopPropagation(); openMaterialForm(Number(button.dataset.materialIndex)); }));
  document.querySelectorAll('.material-delete').forEach(button => button.addEventListener('click', event => { event.stopPropagation(); requestDeletion('material', Number(button.dataset.materialIndex)); }));
}
...
function closeEditor() { updateEditorMeta(); renderBookControls(); renderChapters(); if (isDesktopLayout()) { editorPage.classList.remove('is-visible'); screen.classList.remove('show-editor', 'desktop-editor-open'); chaptersPage.classList.add('is-visible'); screen.classList.add('show-chapters'); } else { showPage('章节'); chaptersPage.classList.add('is-visible'); screen.classList.add('show-chapters'); } if (updateReloadQueued) window.location.reload(); }
...
document.querySelector('#itemPreviewClose').addEventListener('click', closeItemPreview);
document.querySelector('#itemPreview').addEventListener('click', event => { if (event.target.id === 'itemPreview') closeItemPreview(); });
