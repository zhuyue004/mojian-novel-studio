...
document.addEventListener('keydown', event => { if (!isDesktopLayout()) return; if ((event.ctrlKey || event.metaKey) && event.key.toLowerCase() === 's') { event.preventDefault(); if (editorPage.classList.contains('is-visible')) { updateEditorMeta(); notify('已保存'); } } if (event.key === 'Escape') { if (!document.querySelector('#deleteModal').hidden) closeDeleteModal(); else if (editorPage.classList.contains('is-visible')) closeEditor(); else { bookMenu.hidden = true; toolBookMenu.hidden = true; } } });
