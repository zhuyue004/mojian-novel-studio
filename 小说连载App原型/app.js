...
function openCharacterForm() {
  ...
  select.innerHTML = '<option value="">暂无关联 / 核心角色</option>' + scoped('characters').map(item => `<option>${escapeHtml(item.name)}</option>`).join('');
  document.querySelector('#characterFormContext').textContent = contextName();
...
function restoreTrash(id) {
  ...
  else { if (!books[item.workName]) { notify('请先还原所属作品'); return; } const list = item.type === 'chapter' ? books[item.workName].chapters : item.type === 'character' ? books[item.workName].characters : item.type === 'event' ? books[item.workName].events : books[item.workName].materials; list.splice(Math.min(item.index ?? list.length, list.length), 0, item.data); }
...
function requestDeletion(type, itemIndex = null) {
  if (!activeBook || (type === 'chapter' && activeChapterIndex === null) || ((type === 'material' || type === 'character' || type === 'event') && itemIndex === null)) return;
  const code = String(Math.floor(1000 + Math.random() * 9000));
  pendingDeletion = { type, code, book: activeBook, chapter: activeChapterIndex, item: itemIndex };
  document.querySelector('#deleteTitle').textContent = type === 'work' ? '删除这部作品？' : type === 'chapter' ? '删除这一章？' : type === 'character' ? '删除这个人物？' : type === 'event' ? '删除这个事件？' : '删除这条素材？';
  document.querySelector('#deleteHint').textContent = type === 'work' ? `《${activeBook}》及其全部章节、素材和创作资料将被移入回收站。` : type === 'chapter' ? '这一章的标题和正文将被移入回收站。' : type === 'character' ? '这个人物会从谱系图中移除并移入回收站。' : type === 'event' ? '这个事件将被移入回收站。' : '这条素材及其标签将被移入回收站。';
...
function confirmDeletion() {
  ...
  if (pendingDeletion.type === 'work') {
    ...
    const key = pendingDeletion.type === 'character' ? 'characters' : pendingDeletion.type === 'event' ? 'events' : 'materials';
    ...
    saveBooks(); closeDeleteModal(); pendingDeletion.type === 'character' ? renderCharacters() : pendingDeletion.type === 'event' ? renderTimeline() : renderMaterials(); notify(pendingDeletion.type === 'character' ? '人物已删除' : pendingDeletion.type === 'event' ? '事件已删除' : '素材已删除');
  }
}
...
document.querySelector('#characterForm').addEventListener('submit', event => { event.preventDefault(); if (!ensureActiveBook()) return; scoped('characters').push({ name: document.querySelector('#characterName').value.trim(), role: document.querySelector('#characterRole').value.trim(), relatedTo: document.querySelector('#characterRelated').value, relation: document.querySelector('#characterRelation').value.trim(), note: document.querySelector('#characterNote').value.trim() }); saveBooks(); renderCharacters(); event.target.reset(); openTool('#charactersPage'); notify('角色已加入人物谱系图'); });
