...
function renderTimeline() { const items = scoped('events'); document.querySelector('#timelineWorkContext').textContent = contextName(); document.querySelector('#timelineForm').hidden = true; document.querySelector('#timelineList').innerHTML = activeBook && items.length ? `<div class="timeline-rail">${items.map(item => `<article class="timeline-item"><span class="timeline-dot" aria-hidden="true"></span><div class="timeline-content"><time>${escapeHtml(item.time)}</time><h3>${escapeHtml(item.title)}</h3><p>${escapeHtml(item.note || '暂无补充说明')}</p></div></article>`).join('')}</div>` : `<div class="timeline-empty"><b>⌇</b>${activeBook ? '还没有事件，从第一件关键事开始记录。' : '请先选择一部作品。'}</div>`; document.querySelector('#eventCount').textContent = activeBook ? (items.length ? `${items.length} 条事件` : '还没有事件') : '选择作品后查看'; }
...
document.querySelector('#timelineAddButton').addEventListener('click', () => { const form = document.querySelector('#timelineForm'); form.hidden = false; document.querySelector('#eventTime').focus(); form.scrollIntoView({ behavior: 'smooth', block: 'nearest' }); });
