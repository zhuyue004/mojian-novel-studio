...
document.querySelector('#workForm').addEventListener('submit', event => { event.preventDefault(); const name = document.querySelector('#workTitle').value.trim(); const genre = document.querySelector('#workGenre').value.trim(); if (books[name]) { notify('已存在同名作品'); return; } books[name] = { genre, chapters: [] }; saveBooks(); activeBook = name; renderBookControls(); event.target.reset(); notify(`《${name}》已创建`); });
...
renderBookControls(); renderCharacters(); renderTimeline();
