...
function showWorkDetails(name) {
  ...
  const start = book.createdAt ? new Date(book.createdAt) : new Date(); document.querySelector('#detailDays').textContent = `${start.getMonth() + 1}/${start.getDate()}`; document.querySelector('#detailUpdated').textContent = book.updatedAt ? formatDate(book.updatedAt) : '暂无修改';
}
